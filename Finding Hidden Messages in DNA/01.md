...
- Two strands of DNA unwind and each acts as a template for replication, resulting in two complementary DNA strands.
...
- Gene therapy: Uses genetically engineered viral vectors (mini-genomes) to deliver therapeutic genes into patients' cells. To ensure replication, scientists must identify the ori in these vectors.
...
- We form this as a counting problem. With the increase in ori occurrences, the likelihood that binding will successfully occur increases. It is also less likely that a mutation will disrupt the binding.
...
## Faster Frequent Words Approach
- Implement a more efficient algorithm using a sliding window approach. Instead of calling the 'PatternCount' function for each k-mer, maintain a count of each k-mer as we slide the window across the text using a map or dictionary*.
- Indices of the map: Store the indices of each k-mer in the map to efficiently retrieve the positions of the most frequent k-mers.
```
...
##  Results of FrequentWords
- 4 different 9-mers repeat 3 or more times in a given region: ATGATCAAG, CTTGATCAT, TCTTGATCA, and CTCTTGATC.
- ATGATCAAG and CTCTTGATC are reverse complements of CTTGATCAT and TCTTGATCA.
---
...
- The figure below shows the template strand AGTCGCATAGT and its complementary strand ACTATGCGACT. (Each strand is read in the 5' to 3' direction)
...
- Clump: A k-mer forms a (L, t)-clump if it appears at least `t` times in a window of size `L` in the genome.
